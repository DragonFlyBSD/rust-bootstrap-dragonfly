...
RUSTC_BOOTSTRAP_VERSION=1.48.0
CARGO_BOOTSTRAP_VERSION=1.48.0
RUST_VERSION=1.49.0
...
fixup-vendor() {
	...
	fixup-vendor-patch libc src/unix/bsd/freebsdlike/dragonfly/errno.rs || exit 1
}

RUN info clean extract prepatch fixup-vendor config xbuild xdist inst 2>&1
#RUN xbuild xdist inst 2>&1
